...
public interface ProductBaseRepository extends JpaRepository<ProductBase, String> {
    ...
    @Query(value = "SELECT pb.prod_base_id as prodBaseId , pb.prod_title_th as prodTitleTh , pb.prod_title_eng as prodTitleEng , pb.is_enable as isEnable , pb.is_material_enable as isMaterialEnable , pb.description as description , pb.image as image ,(select  count(pf.prod_form_id) from product_form pf where pf.prod_base_id = pb.prod_base_id) as fromCount FROM product_base pb  WHERE pb.is_delete = false  order by pb.prod_base_id", nativeQuery = true)
